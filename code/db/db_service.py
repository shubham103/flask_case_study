...
def createCustomer(ssnid,name,age, address, city, state):
    ...
    if res1 >= 1:
        ...
        mysql.connection.commit()
        res2 = cur.execute("SELECT * from customer where SSN_ID=%s", ssnid)
        ...
        for k, v in val[0].items():
            if k == 'Customer_ID':
                cid = v
                break
        res2 = cur.execute("INSERT INTO customerstatus(SSN_ID, Customer_ID, Status, Message, Last_Updated) VALUES(%s,%s,%s,%s,%s)""",
                    (ssnid, cid, status, msg, datetime.today().strftime('%Y-%m-%d')))
        l.append(True)
        l.append("")
        mysql.connection.commit()
        cur.close()
...
def createAccount(cid, aType, deposit):
    ...
    if res1 == 0:
        ...
        mysql.connection.commit()
        res2 = cur.execute("SELECT * from account where Customer_ID=%s", cid)
        ...
        for k, v in val[0].items():
            if k == 'Account_ID':
                aid = v
                break
        res2 = cur.execute("""INSERT INTO accountstatus(Customer_ID, Account_ID, Account_Type, Status, Message, Last_Updated) VALUES(%s,%s,%s,%s,%s,%s)""",
                    (cid, aid, aType, status, msg, datetime.today().strftime('%Y-%m-%d')))
        l.append(True)
        l.append("")
        mysql.connection.commit()
        cur.close()
...
def updateCustomer(ssnid, name, address, age):
    cur = mysql.connection.cursor()
    res3 = cur.execute("SELECT * from customer where SSN_ID=%s", ssnid)
    if res3 >= 1:
        ...
        mysql.connection.commit()
        res2 = cur.execute("Update customerstatus SET Customer_ID = %s, Status = %s, Message=%s where SSN_ID=%s", (cid, status, msg, ssnid))
        mysql.connection.commit()
        cur.close()
        l=[]
        if res1 >= 1:
...
def deleteCustomer(ssnid):
    ...
    res2 = cur.execute("UPDATE customerstatus SET Status=%s, Message=%s WHERE SSN_ID=%s",(status, msg, ssnid))
    mysql.connection.commit()
    cur.close()
    l=[]
    if res1 >= 1:
...
def deleteAccount(aid):
    ...
    status = "Deactivated"
    msg = "Account Deleted"
    res2 = cur.execute("UPDATE accountstatus SET Status=%s, Message=%s WHERE Account_ID=%s",(status, msg, aid))
    mysql.connection.commit()
    cur.close()
    l = []
    if res1 >= 1:
...
def statementByNumber(aid,n):
    ...
    if res1 >= 1:
        ...
        error_msg="Statement is not found"
        l.append(False)
        l.append(error_msg)
        return l
